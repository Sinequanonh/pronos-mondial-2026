...
function leaderboard(poolId) {
  const players = db.prepare('SELECT id, name FROM players WHERE pool_id = ? ORDER BY name').all(poolId);
  const finished = db.prepare('SELECT * FROM matches WHERE home_score IS NOT NULL AND away_score IS NOT NULL').all();
