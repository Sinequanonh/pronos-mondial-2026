...
const { CAPTAIN_ENABLED } = require('./flags');
...
// Identique en poules et en élimination directe ; les tirs au but ne comptent pas (un match aux TAB = nul).
function pointsFor(match, ph, pa) {
  if (match.home_score == null || match.away_score == null) return null;
  const hs = match.home_score;
  const as = match.away_score;
  if (ph === hs && pa === as) return 3;                     // score exact
  return Math.sign(ph - pa) === Math.sign(hs - as) ? 1 : 0; // bon résultat (nul/vainqueur), TAB ignorés
}
...
async function leaderboard(poolId) {
  ...
  for (const pr of preds) {
    ...
    // capitaine : ce match double pour ce joueur sur sa journée (si la mécanique est active)
    const cap = CAPTAIN_ENABLED && capByPlayer.get(pr.player_id);
