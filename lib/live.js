...
function activeMatches() {
  const now = Date.now();
  return db.prepare('SELECT * FROM matches WHERE home_score IS NULL').all().filter((m) => {
...
function findOurMatch(evDate, homeName, awayName) {
  const t = Date.parse(evDate);
  return db.prepare('SELECT * FROM matches').all().find((m) =>
...
async function pollLive() {
  ...
  for (const ev of events) {
    ...
    const swapped = m.home === awayName; // au cas où l'ordre diffère
    ...
    if (state === 'in') {
      db.prepare("UPDATE matches SET live_hs=?, live_as=?, live_min=?, live_state='in' WHERE id=?")
        .run(hs, as, clock, m.id);
      ...
      db.prepare("UPDATE matches SET home_score=?, away_score=?, advancer=COALESCE(?, advancer), live_state='post', live_min=NULL WHERE id=?")
        .run(hs, as, advancer, m.id);
