...
// ESPN uses US Eastern time for its dates; a UTC match at 00:xx–05:xx UTC
  // may appear under the prior calendar day on ESPN's scoreboard.
  // So we fetch both the UTC date and the previous day for each active match.
  const days = [...new Set(active.flatMap((m) => {
    const utcDay = m.date_utc.slice(0, 10).replace(/-/g, '');
    const prev = new Date(m.date_utc);
    prev.setUTCDate(prev.getUTCDate() - 1);
    const prevDay = prev.toISOString().slice(0, 10).replace(/-/g, '');
    return [utcDay, prevDay];
  }))];
  const events = [];
  for (const d of days) {
